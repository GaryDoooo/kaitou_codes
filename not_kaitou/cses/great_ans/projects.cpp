...
int main() {
    ...
    for (int i = n - 1; i >= 0; --i)
        dp[i] = max(projects[i].p + 
dp[upper_bound(projects.begin(), projects.end(), projects[i].b, 
[](int l, const project &r) {return l < r.a;}) // No need construct a struct to cmp 
// compare a int to a struct a is start time b is end time
//
                                      - projects.begin()], dp[i + 1]);
